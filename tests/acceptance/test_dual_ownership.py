...
def test_RUNM_001_observe_mode_stays_read_only_for_existing_batwing_operations() -> None:
    policy = OwnershipPolicy(project_id="morpheus-batwing")
    owned = ResourceIdentity(
        kind=ResourceKind.CONTAINER,
        name="morpheus-agent",
        labels={"io.morpheus.project": "morpheus-batwing"},
    )
    external = ResourceIdentity(kind=ResourceKind.CONTAINER, name="qwopus-coder", labels={})
