...
def test_ACCESS_003_benchmark_claim_comes_from_completed_runs(tmp_path: Path) -> None:
    ...
    benchmark_claim = next(
        claim for claim in ubuntu_one["claims"] if claim["dimension"] == "benchmark"
    )
...
def test_ACCESS_003_target_named_evidence_is_advertised(tmp_path: Path) -> None:
    ...
    ubuntu_one = targets["ubuntu-1"]
    proven = {claim["dimension"] for claim in ubuntu_one["claims"] if claim["state"] == "proven"}
    assert {
        "os",
        "architecture",
        "accelerator",
    ...
    os_claim = next(claim for claim in ubuntu_one["claims"] if claim["dimension"] == "os")
