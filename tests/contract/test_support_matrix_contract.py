...
def test_ACCESS_003_dev_evidence_never_names_physical_targets(tmp_path: Path) -> None:
    ...
    assert targets == {"ubuntu-1": "unproven", "ubuntu-2": "unproven"}
...
def test_ACCESS_003_target_named_evidence_is_advertised(tmp_path: Path) -> None:
    ...
    assert targets["ubuntu-1"]["state"] == "proven"
    assert targets["ubuntu-1"]["evidence_refs"][0].startswith("diag-ubuntu-1:")
    assert targets["ubuntu-2"]["state"] == "unproven"
