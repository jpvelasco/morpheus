...
for headless use. Desktop and backend updates are signed, independently
versioned, compatibility-checked, health-gated, and recoverable.

## Consequences

- React feature code, API contracts, and browser accessibility tests remain
  shared between desktop and browser delivery.
- Native packaging, signing, notarization, update, and service lifecycle require
  separate Windows, Linux, and macOS build and validation lanes.
