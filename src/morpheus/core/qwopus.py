"""Checksummed Qwopus JSONL import with explicit limitation mapping (BENCH-003).

Accepted line contract (qwopus v1 history shape): one JSON object per line with
...
def parse_qwopus_line(line: str) -> tuple[QwopusLine | None, str | None]:
...
def import_qwopus(
    ...
    context: QwopusImportContext,
) -> QwopusImportReport:
    """Import a Qwopus JSONL history into the store without rewriting it."""
    store.initialize()
    ...
    for raw_line in stream:
        ...
        parsed, limitation = parse_qwopus_line(line)
        ...
        if entry is None:
            ...
            run_id = f"qwopus-{slug}-{hashlib.sha256(key.encode()).hexdigest()[:12]}"
